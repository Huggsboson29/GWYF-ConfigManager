...
namespace ConfigManager.Configuration;
...
public sealed class ActiveSettings
{
    ...
    public const float FallbackVanillaDayDurationSeconds = 300f;
    public const int FallbackVanillaDaysBeforeQuota = 3;
    public const long FallbackVanillaStartingQuota = 100L;
    public const float FallbackVanillaCatchUpFactor = 0.75f;
    ...
    {
        var vanillaDefaults = LoadVanillaDefaults();
        ...
        var activeProfileName = config.Bind(
            ...
            "BepInEx/config/ConfigManager/profiles/.");
    ...
    {
        var gameSettingsType = System.Type.GetType("GameSettings, Assembly-CSharp", throwOnError: false);
        var resourcesType = System.Type.GetType("UnityEngine.Resources, UnityEngine.CoreModule", throwOnError: false);
        if (gameSettingsType != null && resourcesType != null)
        {
            var loadMethod = resourcesType.GetMethod(
                "Load",
                System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Static,
                binder: null,
                types: new[] { typeof(string), typeof(System.Type) },
                modifiers: null);

            var gameSettings = loadMethod?.Invoke(obj: null, parameters: new object[] { "GameSettings", gameSettingsType });
            if (gameSettings != null)
            {
                return new TimingProfile(
                    "Vanilla",
                    true,
                    ReadMember(gameSettings, "dayDuration", FallbackVanillaDayDurationSeconds),
                    ReadMember(gameSettings, "daysBeforeQuota", FallbackVanillaDaysBeforeQuota),
                    ReadMember(gameSettings, "startingQuota", FallbackVanillaStartingQuota),
                    ReadMember(gameSettings, "catchUpFactor", FallbackVanillaCatchUpFactor),
                    QuotaScalingMode.Vanilla,
                    (ReadMember(gameSettings, "quotas", System.Array.Empty<float>()) ?? System.Array.Empty<float>()).ToArray());
            }
        }
        ...
        return new TimingProfile(
            "Vanilla",
            true,
            FallbackVanillaDayDurationSeconds,
            FallbackVanillaDaysBeforeQuota,
            FallbackVanillaStartingQuota,
            FallbackVanillaCatchUpFactor,
            QuotaScalingMode.Vanilla,
    ...
    private static T ReadMember<T>(object instance, string name, T fallback)
    {
        ...
        if (property?.GetValue(instance) is System.IConvertible convertible)
        {
            return (T)System.Convert.ChangeType(convertible, typeof(T), CultureInfo.InvariantCulture);
        }
